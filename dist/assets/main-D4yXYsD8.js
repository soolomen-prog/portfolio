...
Präsentationskatalog sowie weitere Werbemittel erstellt.`},{type:"images-two",left:"/img/has/06.jpg",right:"/img/has/07.jpg"},{type:"image-full",src:"/img/has/08.jpg"},{type:"image-full",src:"/img/has/09.jpg"},{type:"image-full",src:"/img/has/10.jpg"},{type:"image-full",src:"/img/has/11.jpg"}]},j={vakio:b,dolcevita:y,has:w};function z(e){const t=j[e];if(!t){console.warn(`Case "${e}" not found`);return}x(t),S(t),E(t),L(t.blocks)}function x(e){const t=document.querySelector(".case-title-text");t&&(t.textContent=e.header.title)}function S(e){const t=document.querySelector(".case-intro-title"),n=document.querySelector(".case-intro-features"),i=document.querySelector(".case-intro-year");t&&(t.textContent=e.header.subtitle),n&&(n.textContent=e.header.info),i&&(i.textContent=e.header.year);const s=document.querySelector(".case-intro-cover"),r=document.querySelector(".case-intro-image");s&&(s.innerHTML=`<img src="${e.intro.cover}" alt="">`),r&&(r.innerHTML=`<img src="${e.intro.image}" alt="">`)}function E(e){const t=document.querySelector(".case-task-heading"),n=document.querySelector(".case-task-text");t&&(t.textContent=e.task.title),n&&(n.textContent=e.task.text)}function L(e){const t=document.querySelector(".case-admin");t&&(t.innerHTML="",e.forEach(n=>{let i=null;switch(n.type){case"text-right":i=P(n);break;case"images-two":i=I(n);break;case"image-full":i=C(n);break;case"video-full":i=D(n);break;default:console.warn("Unknown block type:",n.type)}i&&t.appendChild(i)}))}function P(e){const t=document.createElement("section");t.className="case-admin-row";const n=e.text.split(/\n\s*\n/).map(i=>i.replace(/\n/g," ").trim()).filter(Boolean).map(i=>`<p>${i}</p>`).join("");return t.innerHTML=`
  ...
  `}function W(){const e=document.getElementById("web-projects"),t=document.getElementById("branding-projects");if(!e||!t)return;const n=c.filter(i=>i.visibleOnHome);n.filter(i=>i.category==="web").sort((i,s)=>i.order-s.order).forEach(i=>e.insertAdjacentHTML("beforeend",h(i))),n.filter(i=>i.category==="branding").sort((i,s)=>i.order-s.order).forEach(i=>t.insertAdjacentHTML("beforeend",h(i))),T()}const o=document.getElementById("case-panel"),l=document.getElementById("case-overlay");function p(e,t){g(t.id),o.classList.remove("hidden","from-left","from-right","active"),l.classList.remove("hidden","from-left","from-right","active");const n=e==="left"?"from-left":"from-right";o.classList.add(n),l.classList.add(n),o.offsetHeight,o.classList.add("active"),l.classList.add("active"),setTimeout(()=>{const a=o.querySelector(".case-inner");a&&(a.scrollTop=0)},0);const i=c.filter(a=>a.category===t.category).sort((a,d)=>a.order-d.order),s=i.findIndex(a=>a.id===t.id),r=i[(s+1)%i.length];if(r){document.querySelector(".case-next-title").textContent=r.title,document.querySelector(".case-next-features").textContent=r.features.join(" / "),document.querySelector(".case-next-year").textContent=r.year;const a=document.querySelector(".case-next-image"),d=document.querySelector(".case-next-link");a.innerHTML=`<img src="${r.coverImage}" alt="">`;const m=v=>{v.preventDefault(),p(r.category==="web"?"left":"right",r)};d.onclick=m,a.onclick=m}o.offsetHeight,o.classList.add("active"),l.classList.add("active")}function k(){f(),o.classList.remove("active"),l.classList.remove("active"),setTimeout(()=>{o.classList.add("hidden"),l.classList.add("hidden")},450)}l.addEventListener("click",k);document.addEventListener("click",e=>{e.target.closest(".case-close")&&k()});function T(){document.querySelectorAll(".project-card").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();const n=e.dataset.id,i=e.dataset.side,s=c.find(r=>r.id===n);s&&p(i,s)})})}W();K();const H={nav:{web:"Webdesign",branding:"Branding",letsTalk:"Let’s talk",about:"Über mich"},project:{details:"Details",close:"Projekt schließen"}};window.t=H;function A(){const e=document.querySelector(".site-header");if(!e)return;const t=e.querySelector(".header-right .nav-item"),n=e.querySelector(".author-name"),i=window.location.pathname,s=i==="/about.html"||i==="/about"||i.endsWith("/about.html");if(t&&(s?(t.textContent="Projekte",t.href="/"):(t.textContent="About",t.href="/about.html")),n&&n.tagName!=="A"){const r=document.createElement("a");r.href="/",r.className="author-name",r.textContent=n.textContent,n.replaceWith(r)}}document.addEventListener("DOMContentLoaded",A);
