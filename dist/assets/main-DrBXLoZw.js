...
Illustrationen sowie der visuellen Darstellung der Produkt-Renderings im Katalog.`},{type:"image-full",src:"/img/vakio/03.jpg"},{type:"video-full",vimeoId:"794384040"},{type:"image-full",src:"/img/vakio/04.jpg"},{type:"text-right",text:`Das finale Design verbindet technische Präzision mit einer klaren visuellen Sprache,
die Vertrauen schafft und die Produkte als hochwertige Lösungen im Bereich Klimatechnik positioniert.`},{type:"image-full",src:"/img/vakio/05.jpg"},{type:"image-full",src:"/img/vakio/06.jpg"},{type:"image-full",src:"/img/vakio/07.jpg"},{type:"text-right",text:`Das Projekt resultierte in einer modernen, skalierbaren Website-Struktur,
die sowohl den Anforderungen von Geschäftskunden als auch von Privatkunden gerecht wird.`},{type:"image-full",src:"/img/vakio/08.jpg"}]},p={vakio:v};function k(e){const t=p[e];if(!t){console.warn(`Case "${e}" not found`);return}y(t),b(t),w(t),x(t.blocks)}function y(e){const t=document.querySelector(".case-title-text");t&&(t.textContent=e.header.title)}function b(e){const t=document.querySelector(".case-intro-title"),n=document.querySelector(".case-intro-features"),i=document.querySelector(".case-intro-year");t&&(t.textContent=e.header.subtitle),n&&(n.textContent=e.header.info),i&&(i.textContent=e.header.year);const a=document.querySelector(".case-intro-cover"),r=document.querySelector(".case-intro-image");a&&(a.innerHTML=`<img src="${e.intro.cover}" alt="" />`),r&&(r.innerHTML=`<img src="${e.intro.image}" alt="" />`)}function w(e){const t=document.querySelector(".case-task-heading"),n=document.querySelector(".case-task-text");t&&(t.textContent=e.task.title),n&&(n.textContent=e.task.text)}function x(e){const t=document.querySelector(".case-admin");t&&(t.innerHTML="",e.forEach(n=>{let i=null;switch(n.type){case"text-right":i=j(n);break;case"image-full":i=S(n);break;case"video-full":i=L(n);break;default:console.warn("Unknown block type:",n.type)}i&&t.appendChild(i)}))}function j(e){const t=document.createElement("section");return t.className="case-intro-row",t.innerHTML=`
  ...
  `,t}function L(e){const t=document.createElement("section");return t.className="case-intro-row",t.innerHTML=`
    <div class="case-col-left"></div>
    <div class="case-col-right">
  ...
  `}function E(){const e=document.getElementById("web-projects"),t=document.getElementById("branding-projects");if(!e||!t)return;const n=d.filter(i=>i.visibleOnHome);n.filter(i=>i.category==="web").sort((i,a)=>i.order-a.order).forEach(i=>e.insertAdjacentHTML("beforeend",g(i))),n.filter(i=>i.category==="branding").sort((i,a)=>i.order-a.order).forEach(i=>t.insertAdjacentHTML("beforeend",g(i))),I()}const o=document.getElementById("case-panel"),c=document.getElementById("case-overlay");function m(e,t){k(t.id),o.classList.remove("hidden","from-left","from-right","active"),c.classList.remove("hidden","from-left","from-right","active");const n=e==="left"?"from-left":"from-right";o.classList.add(n),c.classList.add(n);const i=d.filter(s=>s.category===t.category).sort((s,l)=>s.order-l.order),a=i.findIndex(s=>s.id===t.id),r=i[(a+1)%i.length];if(r){document.querySelector(".case-next-title").textContent=r.title,document.querySelector(".case-next-features").textContent=r.features.join(" / "),document.querySelector(".case-next-year").textContent=r.year;const s=document.querySelector(".case-next-image"),l=document.querySelector(".case-next-link");s.innerHTML=`<img src="${r.coverImage}" alt="">`;const u=h=>{h.preventDefault(),m(r.category==="web"?"left":"right",r)};l.onclick=u,s.onclick=u}o.offsetHeight,o.classList.add("active"),c.classList.add("active")}function f(){o.classList.remove("active"),c.classList.remove("active"),setTimeout(()=>{o.classList.add("hidden"),c.classList.add("hidden")},450)}c.addEventListener("click",f);document.addEventListener("click",e=>{e.target.closest(".case-close")&&f()});function I(){document.querySelectorAll(".project-card").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();const n=e.dataset.id,i=e.dataset.side,a=d.find(r=>r.id===n);a&&m(i,a)})})}E();const C={nav:{web:"Webdesign",branding:"Branding",letsTalk:"Let’s talk",about:"Über mich"},project:{details:"Details",close:"Projekt schließen"}};window.t=C;function P(){const e=document.querySelector(".site-header");if(!e)return;const t=e.querySelector(".header-right .nav-item"),n=e.querySelector(".author-name"),i=window.location.pathname,a=i==="/about.html"||i==="/about"||i.endsWith("/about.html");if(t&&(a?(t.textContent="Projekte",t.href="/"):(t.textContent="About",t.href="/about.html")),n&&n.tagName!=="A"){const r=document.createElement("a");r.href="/",r.className="author-name",r.textContent=n.textContent,n.replaceWith(r)}}document.addEventListener("DOMContentLoaded",P);
