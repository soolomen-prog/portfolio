...
Daraufhin wurden drei unterschiedliche Designkonzepte für die Website entwickelt, von denen das stärkste und für das Projekt am besten geeignete ausgewählt wurde – ein minimalistisches Konzept mit klaren Bildern und einer Dominanz von Weiß`},{type:"image-full",src:"/img/dolcevita/01.jpg"},{type:"image-full",src:"/img/dolcevita/02.jpg"},{type:"image-full",src:"/img/dolcevita/03.jpg"},{type:"text-right",text:"Zur Verbesserung der Benutzerfreundlichkeit ist auf der Website der Kosmetikklinik ein Mini-Onlineshop für spezialisierte Kosmetik integriert, mit einem Katalog, Produktseiten sowie einer Funktion zum Kauf und zur Bestellabwicklung"},{type:"image-full",src:"/img/dolcevita/04.jpg"},{type:"image-full",src:"/img/dolcevita/05.jpg"}]},w={vakio:b,dolcevita:y};function x(e){const t=w[e];if(!t){console.warn(`Case "${e}" not found`);return}j(t),S(t),z(t),E(t.blocks)}function j(e){const t=document.querySelector(".case-title-text");t&&(t.textContent=e.header.title)}function S(e){const t=document.querySelector(".case-intro-title"),n=document.querySelector(".case-intro-features"),i=document.querySelector(".case-intro-year");t&&(t.textContent=e.header.subtitle),n&&(n.textContent=e.header.info),i&&(i.textContent=e.header.year);const s=document.querySelector(".case-intro-cover"),r=document.querySelector(".case-intro-image");s&&(s.innerHTML=`<img src="${e.intro.cover}" alt="">`),r&&(r.innerHTML=`<img src="${e.intro.image}" alt="">`)}function z(e){const t=document.querySelector(".case-task-heading"),n=document.querySelector(".case-task-text");t&&(t.textContent=e.task.title),n&&(n.textContent=e.task.text)}function E(e){const t=document.querySelector(".case-admin");t&&(t.innerHTML="",e.forEach(n=>{let i=null;switch(n.type){case"text-right":i=L(n);break;case"images-two":i=P(n);break;case"image-full":i=C(n);break;case"video-full":i=I(n);break;default:console.warn("Unknown block type:",n.type)}i&&t.appendChild(i)}))}function L(e){const t=document.createElement("section");t.className="case-admin-row";const n=e.text.split(/\n\s*\n/).map(i=>i.replace(/\n/g," ").trim()).filter(Boolean).map(i=>`<p>${i}</p>`).join("");return t.innerHTML=`
  ...
  `,t}let u=!1;function m(e){if(console.log("OPEN CASE ROUTER:",e),!l.find(n=>n.id===e)){console.warn("CASE NOT FOUND:",e);return}history.pushState({caseId:e},"",`/#case/${e}`),x(e),u=!0}function h(){u&&(history.pushState({},"","/"),u=!1)}function D(){const e=location.hash.match(/^#case\/(.+)$/);e&&m(e[1])}window.addEventListener("popstate",()=>{const e=location.hash.match(/^#case\/(.+)$/);e?m(e[1]):h()});function f(e){return`
    <article 
      class="project-card"
