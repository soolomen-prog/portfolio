...
Illustrationen sowie der visuellen Darstellung der Produkt-Renderings im Katalog`},{type:"image-full",src:"/img/vakio/03.jpg"},{type:"video-full",vimeoId:"794384040"},{type:"image-full",src:"/img/vakio/04.jpg"},{type:"text-right",text:"Eine leichte, klare und verständliche Produktseite vermittelt ein Gefühl von Innovation und modernen Technologien. Jede Produktseite ist eine kurze Geschichte über die Werte und Vorteile des Produkts, ergänzt durch Visualisierungen, die zeigen, wie das Produkt in einem realen Wohninterieur aussieht"},{type:"image-full",src:"/img/vakio/05.jpg"},{type:"image-full",src:"/img/vakio/06.jpg"},{type:"image-full",src:"/img/vakio/07.jpg"},{type:"text-right",text:`Um die Einsatz- und Anwendungsschemata der Geräte in Wohnräumen sowie in Geschäftsräumen anschaulich darzustellen, wurde ein einheitlicher Visualisierungsstil für diese схемы entwickelt. In einer stilisierten und minimalistischen 3D-Darstellung wurden die korrekten Platzierungsschemata der Geräte in den Räumen gezeigt.
      ...
      Darüber hinaus wurde ein eigener grafischer Corporate-Stil entwickelt, der in Anleitungen sowie in verschiedenen Gestaltungselementen der Website eingesetzt wird — sei es in Blogartikeln oder auf Informationsseiten`},{type:"image-full",src:"/img/vakio/08.jpg"}]},b={vakio:y};function w(e){const t=b[e];if(!t){console.warn(`Case "${e}" not found`);return}x(t),j(t),S(t),E(t.blocks)}function x(e){const t=document.querySelector(".case-title-text");t&&(t.textContent=e.header.title)}function j(e){const t=document.querySelector(".case-intro-title"),n=document.querySelector(".case-intro-features"),i=document.querySelector(".case-intro-year");t&&(t.textContent=e.header.subtitle),n&&(n.textContent=e.header.info),i&&(i.textContent=e.header.year);const a=document.querySelector(".case-intro-cover"),r=document.querySelector(".case-intro-image");a&&(a.innerHTML=`<img src="${e.intro.cover}" alt="">`),r&&(r.innerHTML=`<img src="${e.intro.image}" alt="">`)}function S(e){const t=document.querySelector(".case-task-heading"),n=document.querySelector(".case-task-text");t&&(t.textContent=e.task.title),n&&(n.textContent=e.task.text)}function E(e){const t=document.querySelector(".case-admin");t&&(t.innerHTML="",e.forEach(n=>{let i=null;switch(n.type){case"text-right":i=C(n);break;case"image-full":i=L(n);break;case"video-full":i=I(n);break;default:console.warn("Unknown block type:",n.type)}i&&t.appendChild(i)}))}function C(e){const t=document.createElement("section");t.className="case-admin-row";const n=e.text.split(/\n\s*\n/).map(i=>i.replace(/\n/g," ").trim()).filter(Boolean).map(i=>`<p>${i}</p>`).join("");return t.innerHTML=`
    <div></div>
    <div class="case-admin-text">
      ${n}
    </div>
  `,t}function L(e){const t=document.createElement("section");return t.className="case-admin-row full",t.innerHTML=`
    <img src="${e.src}" alt="">
  `,t}function I(e){const t=document.createElement("section");return t.className="case-admin-row full",t.innerHTML=`
    <iframe
      ...
      allowfullscreen
    ></iframe>
  `,t}let u=!1;function m(e){l.find(n=>n.id===e)&&(history.pushState({caseId:e},"",`/#case/${e}`),w(e),u=!0)}function h(){u&&(history.pushState({},"","/"),u=!1)}function P(){const e=location.hash.match(/^#case\/(.+)$/);e&&m(e[1])}window.addEventListener("popstate",()=>{const e=location.hash.match(/^#case\/(.+)$/);e?m(e[1]):h()});function f(e){return`
    <article 
      class="project-card"
  ...
  `}function z(){const e=document.getElementById("web-projects"),t=document.getElementById("branding-projects");if(!e||!t)return;const n=l.filter(i=>i.visibleOnHome);n.filter(i=>i.category==="web").sort((i,a)=>i.order-a.order).forEach(i=>e.insertAdjacentHTML("beforeend",f(i))),n.filter(i=>i.category==="branding").sort((i,a)=>i.order-a.order).forEach(i=>t.insertAdjacentHTML("beforeend",f(i))),T()}const o=document.getElementById("case-panel"),c=document.getElementById("case-overlay");function p(e,t){m(t.id),o.classList.remove("hidden","from-left","from-right","active"),c.classList.remove("hidden","from-left","from-right","active");const n=e==="left"?"from-left":"from-right";o.classList.add(n),c.classList.add(n);const i=l.filter(s=>s.category===t.category).sort((s,d)=>s.order-d.order),a=i.findIndex(s=>s.id===t.id),r=i[(a+1)%i.length];if(r){document.querySelector(".case-next-title").textContent=r.title,document.querySelector(".case-next-features").textContent=r.features.join(" / "),document.querySelector(".case-next-year").textContent=r.year;const s=document.querySelector(".case-next-image"),d=document.querySelector(".case-next-link");s.innerHTML=`<img src="${r.coverImage}" alt="">`;const g=k=>{k.preventDefault(),p(r.category==="web"?"left":"right",r)};d.onclick=g,s.onclick=g}o.offsetHeight,o.classList.add("active"),c.classList.add("active")}function v(){h(),o.classList.remove("active"),c.classList.remove("active"),setTimeout(()=>{o.classList.add("hidden"),c.classList.add("hidden")},450)}c.addEventListener("click",v);document.addEventListener("click",e=>{e.target.closest(".case-close")&&v()});function T(){document.querySelectorAll(".project-card").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();const n=e.dataset.id,i=e.dataset.side,a=l.find(r=>r.id===n);a&&p(i,a)})})}z();P();const $={nav:{web:"Webdesign",branding:"Branding",letsTalk:"Let’s talk",about:"Über mich"},project:{details:"Details",close:"Projekt schließen"}};window.t=$;function D(){const e=document.querySelector(".site-header");if(!e)return;const t=e.querySelector(".header-right .nav-item"),n=e.querySelector(".author-name"),i=window.location.pathname,a=i==="/about.html"||i==="/about"||i.endsWith("/about.html");if(t&&(a?(t.textContent="Projekte",t.href="/"):(t.textContent="About",t.href="/about.html")),n&&n.tagName!=="A"){const r=document.createElement("a");r.href="/",r.className="author-name",r.textContent=n.textContent,n.replaceWith(r)}}document.addEventListener("DOMContentLoaded",D);
